...
NOTE: I need to make graphical representations of the findings from the
...
have implemented their own systems and other, non-insitute related systems
...
uses Apache Solr\footnote{\url{http://lucene.apache.org/solr/}} ot index its
...
Hydra project is very flexible - it is used around the world for insitutional
...
iRODS is implemented in C++ and it offers clientAPIs for multiple programming
...
different persitent identifier methods - Zenodo has chosen to use DOI and
EUDAT B2Share used Handle out of the box.

Invenio is like Dataverse or Hydra - a tool to publish digital assets but not
...
version control software. GitHub also hosts some data and vizualises that data.
...
by rewarding the contributors with reckognition.
...
In Sections \ref{sec:expert_interviews} - \ref{sec:csc} the current sitation
...
findings. This list might not be copmrehensive but does contain the biggest
insitutions. It's is likely that other insitutions and the insitutions listed
below are working on more matters. All Finnish universities maintain a digital
publishin archive for papers and theses published in those universities - they
...
\begin{tabularx}{\textwidth}{| >{\raggedright}p{3cm} | X |}
  ...
  National Library of Finland               & The National Library manages the URN identifier scheme that can be
                                              ...
                                              project (PAS). It aslo runs Finna\footnotemark, a digital archive for
...
techincal problem. Perfeclty fine techincal solution for publishing research
...
from creation to archiving - does noe exist. It seems that the challenge
...
this collaboration has not been figured out yet. Without proper techincal
...
institutions in Finland, let alone in the world. Succesful research data
...
needs. Similarly iRODS, the tool for managing resaerch data, is open source. In
addition to using similar tools as their peers different insitutions should
